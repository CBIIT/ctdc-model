...
**Model files:** YAML definitions and version.md
...
## 4. End-to-End Contribution Workflow
...
4.  **Make your changes locally** (YAML edits, docs, etc.).\
    \
    Typical changes may include:

    - Updating YAML model definitions

    - Updating Navigator resources (icons/config)

    - Updating docs/release notes

5.  **Always update the CTDC model version**:\
    \
    To trigger downstream integration and ensure the UI displays the
    correct version, update the CTDC model version in:

    - model-desc/ctdc_model_file.yaml (this change drives downstream
      Data Hub detection)

6.  **Update the display version** in:

    - version.md (displayed in Data Model Navigator "Version" tab)
...
14. Downstream verification in **crdc-datahub-models**
    ...
    - **Verify downstream detection** by checking the prod branch in
      \`crdc-datahub-models\` to ensure the new version commit is
      reflected.

...
A relationship is defined between two nodes, with one node being the Source (Parent) and the other node being the Destination (Child).

### Hierarchy: Parent nodes versus Child nodes

Nodes within the model are connected to each other through relationships in a similar way to a family tree.  A relationship is defined by a source node (Parent) and a Destination Node (Child).  This tells the model how the nodes relate to each other, and which comes first in the hierarchy.  Like a family tree, a node can be both a Parent and a Child at the same time depending on where it exists in the model, these types of nodes are typically in the center of the model.  It is possible for a node to not have any Parents and these are referred to as Root Nodes and are typically at the head or top of the model.  Similarly there are nodes that do not have Children and these are endpoints of the model.  While not all Child nodes will be populated depending on the data, if a child node does have data, then all corresponding parent nodes also need to be populated.
...
There are some properties that show up in the model as "Optional Properties" but are not included in the submission templates.  These are known as system generated properties and are created during the data submission process.  The most common one is "crdc\_id" that serves as a unique identifier for a row of data
...
Child Nodes will have additional property(s) that can be found in the relationship tab (either graph view or table view).  These values are necessary to properly link the child node back to its proper parent node(s).  A relationship property has a unique form that will be Parent\_Node\_Name.Parent\_Key\_Property (an example would be study.study\_short\_name).  In order to populate these values, the list of acceptable terms will depend on the values found in the Key Property (mentioned above) of the corresponding parent node.  Depending on the model and node it is possible for a Child node to have multiple Parents.
